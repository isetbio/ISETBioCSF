...
% Visualize the provided optical image
%
% Syntax:
%   visualizeOpticalImage(opticalImage, displayedWavelengths, figNo)
%
% Description:
%    Visualize the provided optical image along the provided displayed
%    wavelengths on the specified figure.
%
% Inputs:
%    opticalImage         - Struct. An optical image structure
%    displayedWavelengths - Vector. A vector of wavelengths to display.
...
%
    ...
    hFig = figure(figNo);
    clf;
    set(hFig, 'Position', [10 10 1300 1024], 'Color', [1 1 1], ...
        'Name', 'Retinal Image');
    subplot(3, 4, 1);
    image(spatialSupportXDegs, spatialSupportYDegs, rgbImage);
    axis 'xy';
    axis 'image'
    ...
    for k = 2:numel(displayedWavelengths)
        ...
        imagesc(spatialSupportXDegs, spatialSupportYDegs, ...
            squeeze(photons(:, :, idx)), photonRange);
        set(gca, 'XTick', [], 'YTick', [], 'FontSize', 14);
        axis 'xy';
        axis 'image'
        title(sprintf('%d nm\n%2.3fE15 photons/sample/sec', ...
            oiWavelengths(idx), mean(photonsAtWavelengthBand(:)) * 1E-15));
    end
    colormap(gray);
    drawnow;
end
