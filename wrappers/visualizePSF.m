...
%
% Description:
%    Using the provided figure, display the PSF for the provided optical
%    image along the requested wavelength(s).
%
% Inputs:
...
%
    ...
    for k = 1:numel(displayedWavelengths)
        ...
        imagesc(psfSupportArcMin, psfSupportArcMin, ...
            squeeze(psf(:, :, idx)));
        hold on;
        plot([0 0], psfRangeArcMin, 'g-');
        plot(psfRangeArcMin, [0 0], 'g-');
        axis 'xy';
        axis 'image';
        set(gca, 'XLim', psfRangeArcMin, 'YLim', psfRangeArcMin, ...
            'XTick', [-20:5:20], 'YTick', [-20:5:20], 'FontSize', 14);
        if (k == 1)
