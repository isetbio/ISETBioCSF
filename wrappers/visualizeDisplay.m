...
% Retrieve wavelength support & SPD of the primaries.
%
% Syntax:
%   visualizeDisplay(display, figNo)
%
% Description:
%    Retrieve the wavelength support and the spectral power distribution of
%    the primaries.
%
% Inputs:
%    display - Struct. A structure describing the display's properties.
...
%
    ...
    ylabel('energy (mWatts)');
    title('Spectral Power Distributions');
    set(gca, 'FontSize', 14);

    subplot(1, 2, 2);
    gammaTable = displayGet(display, 'gamma table');
    plot(1:size(gammaTable, 1), gammaTable(:, 1), 'r-', 'LineWidth', 1.5);
    hold on;
    plot(1:size(gammaTable, 1), gammaTable(:, 2), 'g-', 'LineWidth', 1.5);
    hold on;
    plot(1:size(gammaTable, 1), gammaTable(:, 3), 'b-', 'LineWidth', 1.5);
    hold on;
