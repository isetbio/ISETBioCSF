...
% Replicate the Banks et al data.
% Syntax:
...
%
    ...
    varargout = UnitTest.runValidationRun(...
        @ValidationFunction, nargout, varargin);
end
...
% 4mm pupil -> 1610. Again, I am not quite sure why increasing the pupil
% size helps a tad more than a factor of 2 both here and above.
%
% 4) - Taking out blur. 12/27/16.
% For the 2 mm pupil regular hex mosaic case, taking out the optical blur
% for case 3 above takes sensitivity to 1006, from 750, which is the
% correct direction for 10 cpd.
%
...
% classifyForOneDirectionAndContrast to get the old behavior. It is now set
% to true. I have also subsequently reduced the number of contrasts that
% the routine computes for -- go back to defaults to produce old behavior
% exactly.
%
...
% the threshold you get. Finally changed to 20 cpd from 10 cpd because this
% is a smaller number of cones and thus also makes things run faster.
...
function ValidationFunction(runTimeParams)
    ...
    if (doSimulationWithBanksEtAlmosaicParams)
        ...
        [validationData1, extraData1] = c_BanksEtAlReplicate(params, ...
            ...
            'conePacking', 'hexReg', 'innerSegmentSizeMicrons', 3, ...
            'coneSpacingMicrons', 3.0, ...
            'cyclesPerDegree', 20, 'luminances', 340, 'pupilDiamMm', 2, ...
            'nContrastsPerDirection', 4, 'lowContrast', 0.005, ...
            'highContrast', 0.1, 'contrastScale', 'log');
        ...
        [validationData2, extraData2] = c_BanksEtAlReplicate(params, ...
            ...
            'generatePlots', runTimeParams.generatePlots, ...
            'conePacking', 'hexReg', 'innerSegmentSizeMicrons', 3, ...
            'coneSpacingMicrons', 3.0, 'cyclesPerDegree', 20, ...
            'luminances', 340, 'pupilDiamMm', 4, ...
            'nContrastsPerDirection', 4, 'lowContrast', 0.005, ...
            'highContrast', 0.1, 'contrastScale', 'log');
        validationValue = ...
            validationData2.mlptThresholds.thresholdContrasts;
        ...
        [validationData3, extraData3] = c_BanksEtAlReplicate(params, ...
            ...
            'useScratchTopLevelDirName', true, ...
            'generatePlots', runTimeParams.generatePlots, ...
            'conePacking', 'rect', 'innerSegmentSizeMicrons', 1.5797, ...
            'coneSpacingMicrons', 2, 'cyclesPerDegree', 20, ...
            'luminances', 340, 'pupilDiamMm', 3, ...
            'nContrastsPerDirection', 4, 'lowContrast', 0.005, ...
            'highContrast', 0.1, 'contrastScale', 'log');
        validationValue = ...
            validationData3.mlptThresholds.thresholdContrasts;
        UnitTest.validationData('validationData3', validationValue, ...
            'UsingTheFollowingVariableTolerancePairs', ...
            'validationValue', 1e-6);
        UnitTest.extraData('extraData3', extraData3);
    else
        % Run with the rectangular mosaic and parameters we think match
        % September 2016.
        %   Rectangular mosaic, 2uM pixel size, 1.4uM collecting area.
        %
        %   Pupil size was 3mm because of the bug where it did not actually
        %   get changed.
        %
        ...
        % December 2016. This is lower than it was in September 2016, where
        % it was about 726. This difference turns out to be because we now
        % compute in single rather than double precision, and the log
        % likelihood signal known exactly classifier actually does a bit
        % better with a double precision representation of the mean
        % isomerizations. Along the way to finding this out, I verified
        % that:
        %
        % Although the noise free isomerizations are very close to same
        % between September and December, when we use the September code to
        % classify December response instances, we get the December
        % performance. Thus the difference is in the responses. The
        % difference is subtle - nothing pops out from histogramming the
        % responses to the L cones for the uniform field or for a grating,
        % and the noise free isomerizations are equal to about 1 part in
        % 10^8.
        %
        ...
        % 0.0001) for the September code. Performance for both times points
        % is 50.00% for a 0% contrast stimulus, which it should because
        % then there really is no difference between the two classes being
        % classified.
        %
        % The numbers above are before I switched the maximum likelihood
        % classifier calculation from using poisspdf to a calculation that
        % takes advantage of the analytic form of the Poisson pdf. Set
        % ANALYTIC_LIKELY to false in classifyForOneDirectionAndContrast to
        % get the old behavior. It is now set to true.
        c_BanksEtAlReplicate(params, ...
            'employStandardHostComputerResources', true, ...
            'useScratchTopLevelDirName', true, ...
            'generatePlots', runTimeParams.generatePlots, ...
            'conePacking', 'rect', 'innerSegmentSizeMicrons', 1.4, ...
            'coneSpacingMicrons', 2, 'cyclesPerDegree', 10, ...
            'luminances', 340, 'pupilDiamMm', 3);
    end
