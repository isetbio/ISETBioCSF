...
if (ischar(mosaicParams.conePacking))
    if (strcmp(mosaicParams.conePacking, 'hex'))
        ...
        if (~isfield(mosaicParams, 'marginF'))
            mosaicParams.marginF = [];
        end
        
        fprintf('\nWill generate an ecc-based hex mosaic with:');
        fprintf('\n\t FOVdegs           : %2.2f',mosaicParams.fieldOfViewDegs);
        fprintf('\n\t resamplingF       : %2.0f',mosaicParams.resamplingFactor);
        fprintf('\n\t PositionTolerance : %g',   mosaicParams.latticeAdjustmentPositionalToleranceF);
        fprintf('\n\t DelaunayTolerance : %g',   mosaicParams.latticeAdjustmentDelaunayToleranceF);
        fprintf('\n\t maxGridIterations : %2.0f\n\n',mosaicParams.maxGridAdjustmentIterations);
        
        theMosaic = coneMosaicHex(mosaicParams.resamplingFactor, ...
            'name', mosaicParams.name, ...
            'fovDegs', mosaicParams.fieldOfViewDegs, ...
            'eccBasedConeDensity', true, ...
            'eccBasedConeQuantalEfficiency', mosaicParams.eccBasedConeQuantalEfficiency, ...
