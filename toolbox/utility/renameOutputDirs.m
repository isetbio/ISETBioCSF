...
function renameOutputDirs()
    ...
    srcPattern = '[LM_PLANE]'; destPattern = 'LM';
    renameDirs(rootDir, srcPattern, destPattern);
    
    srcPattern = '[LMS_PLANE]'; destPattern = 'LMS';
    renameDirs(rootDir, srcPattern, destPattern);
    
    srcPattern = '[BACKGROUND]'; destPattern = 'BG';
    renameDirs(rootDir, srcPattern, destPattern);
    
    srcPattern = '[STIM_TEMPORAL]'; destPattern = 'ST';
    renameDirs(rootDir, srcPattern, destPattern);
    
    srcPattern = '[STIM_SPATIAL]'; destPattern = 'SS';
    renameDirs(rootDir, srcPattern, destPattern);
    
    srcPattern = '[OPTICS]'; destPattern = 'O';
    renameDirs(rootDir, srcPattern, destPattern);
    
    srcPattern = '[MOSAIC]'; destPattern = 'M';
    renameDirs(rootDir, srcPattern, destPattern);
...
function renameDirs(rootDir, srcPattern, destPattern)
    ...
    for k = 1:numel(allDirs)
        if (allDirs(k).isdir == true)
            ...
            sourceDirName = allDirs(k).name;
            
            destDirName = strrep(sourceDirName, srcPattern, destPattern);
            
            sepLocations = strfind(destDirName, '/');
            for kk = 2:numel(sepLocations)
                subDir = destDirName(1:(sepLocations(kk)-1));
                if (exist(subDir, 'dir'))
                    ;
                else
                    fprintf('%s does not exist ! Generating it.\n', subDir);
                    mkdir(subDir)
                end
            end
            
            % special chars
            sourceDirName = strrep(sourceDirName, ' ', '\ ');
            sourceDirName = strrep(sourceDirName, '(', '\(');
            sourceDirName = strrep(sourceDirName, ')', '\)');

            destDirName = strrep(destDirName, ' ', '\ ');
            destDirName = strrep(destDirName, '(', '\(');
            destDirName = strrep(destDirName, ')', '\)');
