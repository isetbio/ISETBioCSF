...
function startGUI(targetDir, dataFormat)
    ...
    hFig = figure(1); clf;
    set(hFig, 'Position', [10 10 1670 500], 'MenuBar', 'none', 'Color', [1 1 1]);
        ...
        if (targetDirInfo(k).isdir) && (~strcmp(targetDirInfo(k).name, '.')) && (~strcmp(targetDirInfo(k).name, '..'))
            ...
            for kk = 0:level2FilesNum-1
                ...
                for varIndex = 1:originalVarNames2Num
                    ...
                    varValues{tableRow+kk, originalVarNamesNum+3+varIndex} = val;
                    
                    if (tableRow == 1)
                         varEditables(originalVarNamesNum+3+varIndex) = false;
                         varFormats{originalVarNamesNum+3+varIndex} = 'char';
                         varWidths{originalVarNamesNum+3+varIndex} = 90;
                         varNames{originalVarNamesNum+3+originalVarNames2Num+1} = 'classifier exists';
                         varNames{originalVarNamesNum+3+originalVarNames2Num+2} = 'classifier compute';
                         varNames{originalVarNamesNum+3+originalVarNames2Num+3} = 'visualize';
                         varFormats{originalVarNamesNum+3+originalVarNames2Num+1} = 'logical';
                         varFormats{originalVarNamesNum+3+originalVarNames2Num+2} = 'logical';
                         varFormats{originalVarNamesNum+3+originalVarNames2Num+3} = 'logical';
                         varWidths{originalVarNamesNum+3+originalVarNames2Num+1} = 110;
                         varWidths{originalVarNamesNum+3+originalVarNames2Num+2} = 110;
                         varWidths{originalVarNamesNum+3+originalVarNames2Num+3} = 110;
                         varEditables(originalVarNamesNum+3+originalVarNames2Num+1) = false;
                         varEditables(originalVarNamesNum+3+originalVarNames2Num+2) = true;
                         varEditables(originalVarNamesNum+3+originalVarNames2Num+3) = true;
                    end
                    
                    varValues{tableRow, originalVarNamesNum+3+originalVarNames2Num+1} = true;
                    varValues{tableRow, originalVarNamesNum+3+originalVarNames2Num+2} = false; 
                    varValues{tableRow, originalVarNamesNum+3+originalVarNames2Num+3} = false;
                end
