function hFigsInfo = visualizeResponseInstances(theMosaic, ...
    ...
    hFigs = visualizeNoiseFreeAndFirstInstanceWithActivationProfile(...
             stimData, noStimData, theMosaic, separatePlotsForImageAndProfile);   
         
    if (separatePlotsForImageAndProfile)
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'noiseFreeXYResponseSTIM',...
                'hFig', hFigs{1});  
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'firstInstanceXYResponseSTIM',...
                'hFig', hFigs{2}); 
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'noiseFreeActivationProfileSTIM',...
                'hFig', hFigs{3});  
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'firstInstanceActivationProfileSTIM',...
                'hFig', hFigs{4});
            
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'noiseFreeXYResponseNULL',...
                'hFig', hFigs{5});  
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'firstInstanceXYResponseNULL',...
                'hFig', hFigs{6}); 
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'noiseFreeActivationProfileNULL',...
                'hFig', hFigs{7});  
        hFigsInfo{numel(hFigsInfo)+1} = struct(...
                'filename', 'firstInstanceActivationProfileNULL',...
                'hFig', hFigs{8});
...
function hFigs = visualizeNoiseFreeAndFirstInstanceWithActivationProfile(...
    ...
    hFigsTmp = renderFigure(1222, theMosaic, visualizedResponseInstance, ...
        ...
        separatePlotsForImageAndProfile);   
    ...
    hFigsTmp = renderFigure(1333, theMosaic, visualizedResponseInstance, ...
...
function hFig = renderFigure(figNo, theMosaic, visualizedResponseInstance, ...
    ...
    if (separatePlotsForImageAndProfile)
        ...
        set(hFigTmp, 'Position', [10 10 510 600], 'Color', [1 1 1]);
