...
for iTrial = 1:nTrials
    ...
    if irParams.runFlag
        theBipolarMosaic.compute(theMosaic.os);
        
        if (iTrial == 1); theIR = colorDetectIRConstruct(theBipolarMosaic, irParams); end;
    ...
    if (iTrial == 1)
        ...
        if irParams.runFlag
            theFirstInstance = struct(...
                'theIRSpikes', single(theIR.mosaic{1}.get('spikes downsampled')),...
