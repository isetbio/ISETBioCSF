...
function trialBlockSize = computeTrialBlockSizeForParforLoop(nTrials, coneMosaicPatternSize, coneMosaicActivePatternSize, oiSizeBytes, emPathLength, displayDiagnostics)
    ...
    coneTypesNum = 3;
    computeTempProductsSize = coneMosaicPatternSize*coneTypesNum + coneMosaicPatternSize*emPathLength*sizeOfDoubleInBytes;
    ...
    singleTrialMemoryGBytes = numberOfCores * (computeTempProductsSize + obj_absorptions_memsize + absorptions_memsize + max([oi_memsize obj_currents_memsize+photocurrents_memsize]))/(1024^3);
