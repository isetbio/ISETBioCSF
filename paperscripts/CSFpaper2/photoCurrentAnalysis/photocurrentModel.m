...
%    photonIntegrationTime - Time binwidth for photon  count signal
...
%    2/13/19  NPC   ISETBIO Team, 2019
    ...
    % photon excitation time step
    dtPhotonExcitation = photonIntegrationTime;
    ...
    if (useDefaultImplementation)
        ...
        idx = 1:(round(dtPhotonExcitation/dt)):numel(modelResponse.pRate);
        modelResponse.meanConeExcitationCountSignal = round(modelResponse.pRate(idx) * dtPhotonExcitation);
        modelResponse.noisyConeExcitationTimeAxis = modelResponse.timeAxis(idx);
