  ...
  condIndex = condIndex+1;
    ...
    for condIndex = 1:numel(examinedCond)
        ...
        elseif (strcmp(params.performanceClassifier,'svmV1FilterEnsemble'))
            fNames = fieldnames(examinedCond(condIndex).ensembleFilterParams);
            for fNameIndex = 1:numel(fNames)
                fName = fNames{fNameIndex};
                params.spatialPoolingKernelParams.(fName) = examinedCond(condIndex).ensembleFilterParams.(fName);
            end
        end
