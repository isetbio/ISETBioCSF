...
% Compare the two mosaics specified within the funcion.
%
% Syntax:
%   compareMosaics
%
% Description:
%    Compaure the two mosaics called within this function.
%
% Inputs:
%    None.
%
...
load(fullfile(IBIOColorDetectOutputBaseDir, ...
    ['[c_BanksEtAlPhotocurrentAndEyeMovements]/', ...
    'M_hexPacking_coneSizeUm1.5797_coneSepUmNaN_VariedConeEff_', ...
    'rotationDegs0_eccentricityDegs0.00_LMSdensities0.60_0.30_0.10_', ...
    'FOVdegs0.98x0.98_intTimeMs5_photonNoiseFrozen_osModelLinear_', ...
    'osTimeStepMs0.10_osNoiseFrozen_apBlur1_dark_0_0_0/matfiles/', ...
    't_coneCurrentEyeMovementsResponseInstances/coneMosaic.mat']));

theMosaic = theData;
coneLocsDegs = theMosaic.coneLocsHexGrid * ...
    1e6 / theMosaic.micronsPerDegree;
maxEcc = max(abs(coneLocsDegs(:)));
fprintf('Computing Delaunay triangularization for ISETBio mosaic\n');
trianglesConeMosaicHex = delaunay(squeeze(coneLocsDegs(:, 1)), ...
    squeeze(coneLocsDegs(:, 2)));
...
trianglesBradley = delaunay(squeeze(coneLocsDegsBradley(:, 1)), ...
    squeeze(coneLocsDegsBradley(:, 2)));

fprintf('Computing quality of mosaics\n');
[qConeMosaicHex, qDistConeMosaicHex] = ...
    computeQuality(trianglesConeMosaicHex, coneLocsDegs);
[qBradley, qDistBradley] = ...
    computeQuality(trianglesBradley, coneLocsDegsBradley);

fprintf('Plotting results\n');

subplotPosVectors = NicePlot.getSubPlotPosVectors('rowsNum', 2, ...
    'colsNum', 2, 'heightMargin', 0.07, 'widthMargin', 0.03, ...
    'leftMargin', 0.05, 'rightMargin', 0.01, 'bottomMargin', 0.05, ...
    'topMargin', 0.03);

spaceLims = 0.5 * 0.9 * [-1 1];
spaceTicks = -0.5:0.1:0.5;
...
plot(coneLocsDegs(:, 1), coneLocsDegs(:, 2), 'ko', ...
...
hold off
set(gca, 'XLim', spaceLims, 'YLim', spaceLims, 'XTick', spaceTicks, ...
    'YTick', spaceTicks, 'FontSize', 14, 'Color', [0 0 0]);
axis 'square'
title('ISETBio mosaic');

ax2 = subplot('Position', subplotPosVectors(1, 2).v);
plot(coneLocsDegsBradley(:, 1), coneLocsDegsBradley(:, 2), 'ko', ...
    'MarkerFaceColor', [0.8 0.8 0.8], 'MarkerSize', 6);
hold on
plot([0 0], [-1 1], 'r-', 'LineWidth', 1.5);
plot([-1 1], [0 0], 'r-', 'LineWidth', 1.5);
...
set(gca, 'XLim', qLims, 'YLim', [0 5500], 'XTick', [0.1:0.1:1.0], ...
    'YTick', [0:1000:5000], 'FontSize', 14);
axis 'square';
grid on
title('ISETBio mosaic');
...
%
% Syntax:
%   [xCoords, yCoords] = rectCoords(xo, yo, delta)
%
% Description:
%    Convert a set of polar (angular) coordinates to a set of cartesian
%    (rectangular) coordinates.
%
% Inputs:
%    xo      - Numeric. The Polar X Coordinate.
%    yo      - Numeric. The Polar Y Coordinate.
%    delta   - Numeric. The angle
%
% Outputs:
%    xCoords - Vector. The rectangular X coordinates for a circle.
%    yCoords - Vector. The rectangular Y coordinates for a circle.
%
% Optional key/value pairs:
%    None.
%
...
%
% Syntax:
%   plotFourier(coneLocs, xo, yo, deltaDegs)
%
% Description:
%    Plot the Fourier function for the provided data.
%
% Inputs:
%    coneLocs  - Matrix. A matrix depicting the cone locations.
%    xo        - Numeric. The polar x coordinate.
%    yo        - Numeric. The polar y coordinate.
%    deltaDegs - Numeric. The angle, in degrees.
%
% Outputs:
%    None.
%
...
for k = 1:size(coneLocsSubset, 1)
    mosaicImage(N2 / 2 + coneLocsSubset(k, 1), ...
        N2 / 2 + coneLocsSubset(k, 2)) = 1.0;
end
...
gaussianEnvelope = exp(-0.5 * (X / gaussianEnvelopeSigma) .^ 2) .* ...
    exp(-0.5 * (Y / gaussianEnvelopeSigma) .^ 2);
...
%
% Syntax:
%   plotTriangles(triangles, nodeLocs)
%
% Description:
%    Plot triangles, provided the triangles, and node locations.
%
% Inputs:
%    triangles - Object. A triangular cone mosaic hex object.
%    nodeLocs  - Matrix. A matrix, where the first column are the x
%                coordinates, and the second column are the y coordinates
%                for all of the node locations.
%
% Outputs:
%    None.
%
...
for triangleIndex = 1:trianglesNum 
    plot(X(triangles(triangleIndex, :)), ...
        Y(triangles(triangleIndex, :)), 'w-', 'LineWidth', 1.0);
    if (triangleIndex == 1), hold on; end
end

end

function [qMean, q] = computeQuality(triangles, nodeLocs)
% Compute the image quality.
%
% Syntax:
%   [qMean, q] = computeQuality(triangles, nodeLocs)
%
% Description:
%    Compute the image quality given the triangle mosaics and node locs.
%
% Inputs:
%    triangles - Object. A triangular cone mosaic hex object.
%    nodeLocs  - Matrix. A matrix, where the first column are the x
...
%
% Outputs:
%    qMean     - Numeric. The image's mean quality.
%    q         - Matrix. A matrix recording the quality at each location.
%
% Optional key/value pairs:
%    None.
%
...
for triangleIndex = 1:trianglesNum
    ...
    q(triangleIndex) = (bLength + cLength - aLength) * ...
        (cLength + aLength - bLength) * (aLength + bLength - cLength) / ...
