function generateMosaicWithPSFsuperimposedFig
    ...
    xTicksMeters = xTicksArcMin / 60 * theConeMosaic.micronsPerDegree * 1e-6;
    set(ax,'XTickLabels', xTicksArcMin, 'XTick', xTicksMeters, 'YTickLabels', xTicksArcMin, 'YTick', xTicksMeters);
    xlabel(ax,'arc min');
    set(ax, 'XLim', [xTicksMeters(1) xTicksMeters(end)], 'YLim', [xTicksMeters(1) xTicksMeters(end)]);
    set(gca, 'FontSize', 16);
    xlabel('arc min');
    ylabel('arc min');
    box(ax, 'on')
    ...
    contourf(ax,xGridMinutes, yGridMinutes, psf/max(psf(:)), contourLevels);
    
    axis 'image'; axis 'xy';
    set(gca, 'XLim', [xTicksArcMin(1) xTicksArcMin(end)], 'YLim', [xTicksArcMin(1) xTicksArcMin(end)], 'CLim', [0 1], ...
    ...
    NicePlot.exportFigToPDF(sprintf('%s-%d nm.pdf', opticsModel, targetWavelength), hFig, 300)
end
