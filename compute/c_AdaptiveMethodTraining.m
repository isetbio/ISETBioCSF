...
%
% Key/value pairs
%   'nTrainingSamples' - value (default 1000).  Number of precomputed training samples to
%       draw from.
%   'nAdaptiveTrials' - value (default 100). Number of trials to run the adaptive
%       procedure
...
p.addParameter('nAdaptiveTrials',100,@isnumeric);
...
%
% Some parameters
numTrialsPerStaircase = p.Results.nAdaptiveTrials; 
...
for i = 1:numTrialsPerStaircase
    ...
    for k = 1:nInterleavedStaircases
        ...
        % Simulate trial
        noiseIntervalResponseIndex = randi(testDirectionParams.trialsNum);
        noiseIntervalResponse = noStimData.responseInstanceArray(noiseIntervalResponseIndex).theMosaicIsomerizations(:);
        signalIntervalResponseIndex = randi(testDirectionParams.trialsNum);
...
for k = 1:nInterleavedStaircases
    ...
    plot(xvalues,log10(threshStair(k))*ones(1,numTrialsPerStaircase),colors(k));
end
