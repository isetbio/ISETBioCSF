...
p.addParameter('stimulusOrientationDegs', 0, @isnumeric);
...
p.addParameter('patchSize2SigmaCycles', [], @isnumeric);
...
p.addParameter('parforWorkersNumForClassification', 2, @isnumeric);
...
recordRepoVersion = false;
if (recordRepoVersion)
...
for ll = 1:length(p.Results.luminances)
    for cc = 1:length(p.Results.cyclesPerDegree)
        ...
        %% Find performance
        if (p.Results.findPerformance) ||  (p.Results.visualizeResponsesWithSpatialPoolingSchemeInVideo) || (p.Results.visualizePerformance)
            
            
            if (isempty(p.Results.patchSize2SigmaCycles))
                % Stimulus of Banks et al 1987
                % if we are using a minimumMosaicFOVdegs, adjust  thresholdParams.poolingTemplateWidthInDegrees 
                if (~isempty(p.Results.minimumMosaicFOVdegs)) && (rParams.spatialParams.fieldOfViewDegs < abs(p.Results.minimumMosaicFOVdegs))
                    if (rParams.spatialParams.fieldOfViewDegs < p.Results.minimumMosaicFOVdegs)
                        % Larger mosaic, single template size based on larger mosaic, not the stimulus
                        thresholdParams.poolingTemplateWidthInDegrees = p.Results.minimumMosaicFOVdegs;
                        fprintf(2, 'Performance for stimulus %2.4f c/deg. Using using minimum mosaic (%2.5f degs instead of %2.5f degs)\n)', p.Results.cyclesPerDegree(cc), p.Results.minimumMosaicFOVdegs, rParams.spatialParams.fieldOfViewDegs);
                    else
                ...
                % Stimulus of Crowell & Banks
                extraPoolingRegionForEyeMovementsDegs = 0.0;
                thresholdParams.poolingTemplateWidthInDegrees = min(...
                    [ ...
                        rParams.spatialParams.fieldOfViewDegs+extraPoolingRegionForEyeMovementsDegs ...
                        p.Results.minimumMosaicFOVdegs ...
                    ]);
            end
            ...
            t_colorDetectFindPerformance(...
                ...
                'visualizeResponsesWithSpatialPoolingSchemeInVideo', p.Results.visualizeResponsesWithSpatialPoolingSchemeInVideo, ...
...
function rParams = updateSpatialParams(rParams, userParams, cyclesPerDegree)
    ...
    if (isempty(userParams.patchSize2SigmaCycles))
        % Stimulus of Banks et al 1987
        ...
        rParams.spatialParams = modifyStructParams(rParams.spatialParams, ...
                ...
                'ang', (pi/180)*userParams.stimulusOrientationDegs, ...
        ...
        % Stimulus of Crowell & Banks
        sigmaDegs = 0.5*(userParams.patchSize2SigmaCycles) / cyclesPerDegree;
...
function rParams = updateMosaicParams(rParams, userParams, mosaicFOVdegs)
    ...
    if strcmp(userParams.conePacking, 'hex')
        rParams.mosaicParams = modifyStructParams(rParams.mosaicParams, ...
            ...
            'eccBasedMacularPigment', userParams.eccBasedMacularPigment, ... 
