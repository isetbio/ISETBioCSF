% Plot color stimuli in 3D along color thresholds using Poirson & Wandell.
%
% Description:
...
%
%    If we fix the spatial and temporal properties of a pattern and
%    consider how color thresholds vary with color direction, it is a
%    reasonable empirical generalization that the threshold point will lie
%    on the surface of an ellipsoid in the cone contrast space. The
%    ellipsoidal characterizatoin may not be perfect, but it certainly
%    captures major features of the data across many studies.
%
%    This routine generates and plots such thresholds based on data and a
%    model reported by Poirson & Wandell:
%       Poirson AB, Wandell BA. 1996. Pattern-color separable pathways
%       predict sensitivity to simple colored patterns Vision Res 36:
%       515-26.
%
%    This paper reports measurements of color thresholds in many directions
%    in color space for a variety of spatial frequencies between 0.5 and 8
%    cycles per degree. The paper reports that the data are well-described
%    by ellipsoids, and that the the effect of spatial frequency can be
%    captured by a model that assumes that the effect of pattern (spatial
%    frequency in this case) on thresholds is separable from the effect of
%    color direction.
%
...
%    ellipsoid for each spatial frequency measured. This tutorial
%    recreates Figures 3 and 8 of the paper (but without the data points),
%    and also produces a rotating 3D plot of the full ellipsoid whose
%    sections are shown in Figure 3.
%

% History:
%    06/29/16  dhb  Wrote it.
%    09/16/19  jnm  Documentation pass
...
% The paper gives us (in Tables 2 and 3) the for a matrix A that converts
% cone excitations to a mechanism space in which the threshold contour is a
% unit sphere.
%
...
theSf = 2;
[~, Ainv, Q, theBgLMS] = ...
    PoirsonWandellEllipsoidParameters(conditionStr, theSf);
...
% in the transformed space of a cone-space vector x is given by
%    ||Ax|| = x' * A' * A * x = x' * Q * x.
...
ylabel('M contrast');
zlabel('S contrast');
title({'Poirson Wandell Thrshold Ellipsoid' ''});
xlim([-0.02 0.02]);
ylim([-0.02 0.02]);
zlim([-0.04 0.04]);
axis('square');
...
% This is done by routine PointsOnEllipsoidFind. Here, then, we generate
% circles in the (L, M), (L, S), and (M, S) planes and then adjust the
% length in each direction to give us the points in that plane that are on
% the ellipsoid.
nThetaEllipse = 200;
...
% Since we're on a roll, let's add those. One way to do this is to zero out
% two of the three rows of A, construct the corresponding Q, and proceed as
% above. The code is length because each of the three cases for each
% subplot is just hard-coded here.
%
...
[A, Ainv, Q, theBgLMS] = PoirsonWandellEllipsoidParameters(...
    [psychoEllipsoidParams.subject, ',', ...
    psychoEllipsoidParams.condition], psychoEllipsoidParams.theSf);
nThetaEllipse = 200;
xCircle = UnitCircleGenerate(nThetaEllipse);
xCirclePlane = [xCircle(1, :); xCircle(2, :); zeros(size(xCircle(1, :)))];
xEllipsoidPlane = PointsOnEllipsoidFind(Q, xCirclePlane);
...
xlabel('L contrast', 'FontSize', labelFontSize);
ylabel('M contrast', 'FontSize', labelFontSize);
title('M versus L', 'FontSize', titleFontSize);
xlim([-contrastLim contrastLim]);
ylim([-contrastLim contrastLim]);
axis('square');

rwObject = IBIOColorDetectReadWriteBasic;
writeProgram = mfilename;
paramsList = {psychoEllipsoidParams};
rwObject.write('PWLMThresholdEllipse', hFig, paramsList, ...
    writeProgram, 'Type', 'figure');
