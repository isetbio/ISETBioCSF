...
%
% Description:
%    This is the tutorial described in the Cottaris et al (2019B) paper.
%
...
stimParams = struct(...
    ...
    'contrast', 0.9, ...          % 90% Michelson contrast
...
realizedStimulusScene = ...
    realizeSceneInDisplay(stimulusScene, presentationDisplay);
realizedBackgroundScene = ...
    realizeSceneInDisplay(backgroundScene, presentationDisplay);
...
% Compute responses from -100ms to +200ms around the stimulus onset
stimulusTimeAxisSeconds = -0.1:stimulusSamplingIntervalSeconds:0.2;
stimONbins = stimulusTimeAxisSeconds >= 0 & stimulusTimeAxisSeconds <= ...
    stimulusDurationSeconds - stimulusSamplingIntervalSeconds;
stimulusTemporalModulation = zeros(1, numel(stimulusTimeAxisSeconds));
stimulusTemporalModulation(stimONbins) = 1;

%% Compute the optical image sequence simulating the stimulus presentation
theOIsequence = oiSequence(backgroundOI, stimulusOI, ...
    stimulusTimeAxisSeconds, stimulusTemporalModulation, ...
    'composition', 'blend');
theOIsequence.visualize('montage');

recomputeConeMosaic = false;
if (recomputeConeMosaic)
    %% Generate an ecc-based cone quantal efficiency hexagonal cone mosaic
    % Set the hex lattice sampling factor to 7
    % fovDegs: match mosaic width to stimulus size
    ...
    % maxGridAdjustmentIterations: terminate the iterative lattice
    %     adjustment after 50 iterations.
...
%% Generate the fixational eye movements
% for 1 trial and for the entire simulation time
nTrials = 1;
eyeMovementsNum = ...
    theOIsequence.maxEyeMovementsNumGivenIntegrationTime(...
    theConeMosaic.integrationTime);
theEMPaths = ...
    theConeMosaic.emGenSequence(eyeMovementsNum, 'nTrials', nTrials);

%% Response time axis
responseTimeAxis = (1:eyeMovementsNum)*theConeMosaic.integrationTime + ...
...
for iTrial = 1:nTrials
    [absorptionsCountSequence(iTrial, :, :), ...
        photoCurrentSequence(iTrial, :, :)] = ...
        theConeMosaic.computeForOISequence(theOIsequence, ...
        'emPaths', theEMPaths(iTrial, :, :), 'currentFlag', true);
end
...
visualizedTrial = 1;
visualizedAbsorptions = ...
    squeeze(absorptionsCountSequence(visualizedTrial, :, :));
visualizedPhotocurrents = ...
    squeeze(photoCurrentSequence(visualizedTrial, :, :));
...
for tBin = 1:numel(responseTimeAxis)
    subplot(3, 2, 1);
    plot(visualizedEMPath(1:tBin, 1), -visualizedEMPath(1:tBin, 2), 'k-');
    hold on;
    plot(visualizedEMPath(tBin, 1), -visualizedEMPath(tBin, 2), 'rs');
    hold off;
    ...
    theConeMosaic.renderActivationMap(axHandle, ...
        visualizedAbsorptions(:, tBin), 'mapType', 'modulated disks', ...
        'signalRange', absorptionsRange);

    axHandle = subplot(3, 2, 5);
    theConeMosaic.renderActivationMap(axHandle, ...
        visualizedPhotocurrents(:, tBin), 'mapType', 'modulated disks', ...
        'signalRange', photoCurrentRange);

    subplot(3, 2, 4);

    plot(responseTimeAxis(1:tBin) * 1000, ...
        visualizedAbsorptions(lConeIndices, 1:tBin), 'r-');
    hold on
    plot(responseTimeAxis(1:tBin) * 1000, ...
        visualizedAbsorptions(mConeIndices, 1:tBin), 'g-');
    plot(responseTimeAxis(1:tBin) * 1000, ...
        visualizedAbsorptions(sConeIndices, 1:tBin), 'b-');
    hold off

    set(gca, ...
        'XLim', [responseTimeAxis(1) responseTimeAxis(end)] * 1000, ...
        'YLim', [min(visualizedAbsorptions(:)), ...
        max(visualizedAbsorptions(:))]);
    set(gca, 'FontSize', 16);
    xlabel('time (ms)');
    ...
    subplot(3, 2, 6);
    plot(responseTimeAxis(1:tBin) * 1000, ...
        visualizedPhotocurrents(lConeIndices, 1:tBin), 'r-');
    hold on
    plot(responseTimeAxis(1:tBin) * 1000, ...
        visualizedPhotocurrents(mConeIndices, 1:tBin), 'g-');
    plot(responseTimeAxis(1:tBin) * 1000, ...
        visualizedPhotocurrents(sConeIndices, 1:tBin), 'b-');
    hold off
    set(gca, ...
        'XLim', [responseTimeAxis(1) responseTimeAxis(end)] * 1000, ...
        'YLim', [min(visualizedPhotocurrents(:)), ...
        max(visualizedPhotocurrents(:))]);
    set(gca, 'FontSize', 16);
    xlabel('time (ms)');
    ylabel('pCurrent');
