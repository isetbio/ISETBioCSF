function [validationData, varargout] = ...
    t_plotDetectThresholdsOnLMPlane(varargin)
% Multiple plots based on data from t_colorDetectFindPerformance
%
% Syntax:
%   [validationData, [varargout]] = ...
%       t_plotDetectThresholdsOnLMPlane([varargin])
%
% Description:
...
%
%    A) Plot the psychometric functions with a fit Weibull, which is used
%       to find the threshold in each color direction.
...
%    varargout        - (Optional) VARIES. Additional output as necessary,
%                       in the form of a cell array.
%
% Optional key/value pairs:
%    rParams          - Struct. The is the rParams structure to use.
%                       Default empty, which then uses defaults produced by
...
%                       Default empty, which then uses defaults produced by
%                       a generation function.
%    freezeNoise      - Boolean. Whether to set the rng seed to a value (1)
%                       so output is reproducible. Default true.
%    generatePlots    - Boolean. Whether to make the plots. This must be
%                       true for either plotPsyhometric or plotEllipse to
%                       have an effect. Default true.
%    plotPsychometric - Boolean. Whether to produce psychometric function
%                       output graphs. Default true.
%    plotEllipse      - Boolean. Whether toplot a threshold ellipse in the
%                       LM plane. Default true.
%    delete           - Boolean. Whether to delete all output files after
%                       the function is complete. Not yet implemented.
%                       Default false.
%
...
paramsList = {rParams.topLevelDirParams, rParams.spatialParams, ...
    rParams.temporalParams, rParams.oiParams, rParams.mosaicParams, ...
...
[psychoData, varargout{1}] = t_fitPsychometricFunctions(...
    'rParams', rParams, 'instanceParams', instanceParams, ...
    'thresholdParams', thresholdParams, ...
    'freezeNoise', p.Results.freezeNoise, ...
    'generatePlots', ...
    p.Results.generatePlots && p.Results.plotPsychometric, ...
...
if (any(testConeContrasts(3, :) ~= 0))
    fprintf(['Currently only fitting ellipses in the LM plane ', ...
        'Skipping this step...\n']);
    return;
end
...
for ii = 1:size(thresholdConeContrasts, 2)
    if (~any(isnan(thresholdConeContrasts(:, ii))))
        thresholdConeContrastsForFitting0 = ...
            [thresholdConeContrastsForFitting0, ...
            thresholdConeContrasts(:, ii)];
    end
end

%% Fit ellipse
% This method fits a 3D ellipsoid and extracts the LM plane of the fit.
%
...
thresholdContrastsForFitting = [thresholdConeContrastsForFitting0, ...
    thresholdConeContrastsForFitting1, thresholdConeContrastsForFitting2];
[fitA, fitAinv, fitQ, fitEllParams] = ...
    EllipsoidFit(thresholdContrastsForFitting);

%% Get the LM plane ellipse from the fit
nThetaEllipse = 200;
circleIn2D = UnitCircleGenerate(nThetaEllipse);
circleInLMPlane = [circleIn2D(1, :); circleIn2D(2, :); ...
    zeros(size(circleIn2D(1, :)))];
...
if (p.Results.generatePlots && p.Results.plotEllipse)
    ...
    plot(thresholdConeContrasts(1, :), thresholdConeContrasts(2, :), ...
        'ro', 'MarkerFaceColor', 'r', ...
        'MarkerSize', rParams.plotParams.markerSize);
    plot(ellipsoidInLMPlane(1, :), ellipsoidInLMPlane(2, :), 'r', ...
        'LineWidth', 2);
    plot([-contrastLim contrastLim], [0 0], 'k:', 'LineWidth', 2);
    plot([0 0], [-contrastLim contrastLim], 'k:', 'LineWidth', 2);
    xlabel('L contrast', 'FontSize', rParams.plotParams.labelFontSize);
    ylabel('M contrast', 'FontSize', rParams.plotParams.labelFontSize);
    title('M versus L', 'FontSize', rParams.plotParams.titleFontSize);
    xlim([-contrastLim contrastLim]);
    ylim([-contrastLim contrastLim]);
    axis('square');
    rwObject.write('LMThresholdContour', hFig, paramsList, ...
