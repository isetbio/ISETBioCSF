...
% Illustrate a set of basic cone isomerization calculation steps.
%
% Syntax:
...
%
%    This will produce a Gabor pattern, and the function's subfunctions
%    also document what the relavant parameters are.
%
%    Note: t_coneIsomerizationsMovieSpot calls into this one with
%    parameters that produce a demo for a spot.
%
...
%    rParams            - Struct. The rParams structure to use. Default
%                         is empty, which uses defaults produced by
%                         generation function.
%    isomerizationNoise - String. The noise type. Options are 'random',
%                         'frozen', or 'none'.
%    generatePlots      - Boolean. Whether to generate plots. Default true.
%    setRngSeed         - Boolean. Whether to freeze rng seed to freeze
%                         noise (true). Default true.
%
% See Also:
%   t_coneIsomerizationsMovieSpot, t_coneCurrentEyeMovementsMovie
%   responseParamsGenerate, and colorSceneCreate
%

%% Parse vargin for options passed here
p = inputParser;
p.addParameter('rParams', [], @isemptyorstruct);
p.addParameter('isomerizationNoise', 'none', ...
    @(x)ismember(x, coneMosaic.validNoiseFlags));
...
if (p.Results.generatePlots)
    ...
    plot(rParams.temporalParams.sampleTimes, ...
        rParams.temporalParams.gaussianTemporalWindow, 'r');
    xlabel('Time (seconds)');
...
for ii = 1:rParams.temporalParams.nSampleTimes
    ...
    % window.
    rParamsTemp = rParams;
    rParamsTemp.colorModulationParams.contrast = ...
        rParams.colorModulationParams.contrast * ...
        rParams.temporalParams.gaussianTemporalWindow(ii);
    fprintf('Computing scene %d of %d, time %0.3f, windowVal %0.3f\n', ...
        ii, rParamsTemp.temporalParams.nSampleTimes, ...
        rParamsTemp.temporalParams.sampleTimes(ii), ...
        rParamsTemp.temporalParams.gaussianTemporalWindow(ii));
    gaborScene{ii} = colorSceneCreate(rParamsTemp.spatialParams, ...
        rParams.backgroundParams, rParamsTemp.colorModulationParams, ...
        rParams.oiParams);
end
...
if (p.Results.generatePlots)
    ...
    visualizeSceneOrOpticalImageSequence(rwObject, paramsList, ...
        theProgram, 'scene', gaborScene, ...
        rParams.temporalParams.sampleTimes, showLuminanceMap, ...
        'gaborStimulusMovie');
end

%% Create the OI object for computing the retinal images from the scenes
...
for ii = 1:rParams.temporalParams.nSampleTimes
    ...
    fprintf('Computing optical image %d of %d, time %0.3f\n', ii, ...
        rParams.temporalParams.nSampleTimes, ...
        rParams.temporalParams.sampleTimes(ii));
...
visualizeSceneOrOpticalImageSequence(rwObject, paramsList, theProgram, ...
    'optical image', theOI, rParams.temporalParams.sampleTimes, ...
    showLuminanceMap, 'gaborOpticalImageMovie');
end
...
for ii = 1:rParams.temporalParams.nSampleTimes
    ...
    fprintf('Computing isomerizations %d of %d, time %0.3f\n', ii, ...
        rParams.temporalParams.nSampleTimes, ...
        rParams.temporalParams.sampleTimes(ii));
    gaborConeAbsorptions(:, :, ii) = theMosaic.compute(theOI{ii}, ...
        'currentFlag', false);
end
...
if (p.Results.generatePlots)
    eyeMovementSequence = [];
    visualizeSubMosaicResponseSequence(rwObject, paramsList, ...
        theProgram, 'isomerizations (R*/cone)', gaborConeAbsorptions, ...
        eyeMovementSequence, theMosaic.pattern, ...
        rParams.temporalParams.sampleTimes, ...
        [theMosaic.width theMosaic.height], theMosaic.fov, ...
...
for ii = 1:rParams.temporalParams.nSampleTimes
    LMSContrasts(:, ii) = mosaicUnsignedConeContrasts(...
        gaborConeAbsorptions(:, :, ii), theMosaic);
end
if (p.Results.generatePlots)
