function [validationData, extraData] = ...
    t_colorDetectFindPerformance(varargin)
% Classify the data from t_coneCurrentEyeMovementsResponseInstances
%
% Syntax:
...
%
%    That tutorial generates multiple noisy instances of responses for
%    color Gabors and saves them out. Here we read the output and use an
%    SVM to build a computational observer that gives us percent correct,
%    and then do this for multiple contrasts. The output of this tutorial
%    can then be used to find and plot thresholds by
%      t_colorGaborDetectThresholdsOnLMPlane
%
% Key/value pairs
%    rParams                - Struct. The is the rParams structure to use.
%                             Default empty, which then uses defaults
%                             produced by a generation function.
%    testDirectionParams    - Struct. The testDirectionParams structure to
%                             use. Default empty, which then uses defaults
%                             produced by a generation function.
%    thresholdParams        - Struct. A structure of parameters related to
%                             how we find thresholds from responses.
%                             Default empty, which then uses defaults
%                             produced by a generation function.
%    freezeNoise            - Boolean. Whether to freeze all noise so that
%                             results are reproducible. Default true.
%           Note: If there is no noise set, this leaves it alone.
%    compute                - Boolean. Whether to do the computations.
%                             Default true.
%    parforWorkersNum       - Numeric. How many workers to use for the
%                             computations. Values 0:20. Default 6. Options
%                             for values are:
%           0: for a serial for loop
%           >0: for a parfor loop with desired number of workers
%    generatePlots          - Boolean. Whether to produce any plots. Other
%                             plot variables depend on this setting.
%                             Default true.
%    visualizeSpatialScheme - Boolean. Whether to visualize the
%                             post-receptoral spatial pooling scheme.
%                             Default false.
%    visualizeKernelTransformedSignals
%                           - Boolean. Whether to visualize the output of
%                             the post-receptoral spatial pooling scheme.
%                             Default false.
%    plotPsychometric       - Boolean. Whether to produce psychometric
%                             function output graphs. Default false.
%    plotSvmBoundary        - Boolean. Whether to plot the classification
%                             boundary. Default false.
%    plotPCAAxis1           - Numeric. The first PCA component to plot.
%                             Default 1.
%    plotPCAAxis2           - Numeric. The second PCA component to plot.
%                             Default 2.
%    delete                 - Boolean. Whether to delete the response
%                             instance files. Useful for cleaning up output
%                             when we are done with it. If this is true,
%                             then output files are deleted at the end of
%                             execution. Does not currently delete figures
%                             or parent directories, but maybe it should?
%                             Default false.
%
% See Also:
%   t_coneCurrentEyeMovementsResponseInstances,
...
if (~strcmp(rParams.mosaicParams.conePacking, 'hex')) && ...
        (~strcmp(rParams.mosaicParams.conePacking, 'hexReg')) && ...
        ((strcmp(thresholdParams.method, 'svmV1FilterBank')))
    error(['Currently, classification using the ''svmV1FilterBank'' ', ...
        'method is only implemented for  hex mosaics.\n'])
end

if (~strcmp(rParams.mosaicParams.conePacking, 'hex')) && ...
        (~strcmp(rParams.mosaicParams.conePacking, 'hexReg')) && ...
        ((strcmp(thresholdParams.method, 'svmV1FilterEnsemble')))
    error(['Currently, classification using the ''svmV1FilterEnsemble', ...
        ''' method is only implemented for  hex mosaics.\n'])
end
...
if (~strcmp(rParams.mosaicParams.conePacking, 'hex')) && ...
    ...
    error(['Currently, classification using the ''svmGaussianRF'' ', ...
        'method is only implemented for  hex mosaics.\n'])
end

if (strcmp(thresholdParams.method, 'svmV1FilterBank'))
    % Generate V1 filter bank struct and add it to thresholdParams
    V1filterBank = generateV1FilterBank(rParams.spatialParams, ...
        rParams.mosaicParams, rParams.topLevelDirParams, ...
        p.Results.visualizeSpatialScheme, thresholdParams, ...
        constantParamsList);
    ...
    if (p.Results.visualizeResponsesWithSpatialPoolingSchemeInVideo)
        visualizeResponsesWithSpatialPoolingSchemeInVideo(...
            thresholdParams.method, V1filterBank, ...
            rParams.colorModulationParams, rParams.spatialParams, ...
            rParams.mosaicParams, rParams.topLevelDirParams, ...
            thresholdParams, constantParamsList);
    end
end

if (strcmp(thresholdParams.method, 'svmV1FilterEnsemble'))
    % Generate V1 filter ensemble struct and add it to thresholdParams
    V1filterEnsemble = generateV1FilterEnsemble(rParams.spatialParams, ...
        rParams.mosaicParams, rParams.topLevelDirParams, ...
        p.Results.visualizeSpatialScheme, thresholdParams, ...
        constantParamsList);
    thresholdParams = modifyStructParams(thresholdParams, ...
    ...
    if (p.Results.visualizeResponsesWithSpatialPoolingSchemeInVideo)
        visualizeResponsesWithSpatialPoolingSchemeInVideo(...
            thresholdParams.method, V1filterEnsemble, ...
            rParams.colorModulationParams, rParams.spatialParams, ...
            rParams.mosaicParams, rParams.topLevelDirParams, ...
            thresholdParams, constantParamsList);
    end
...
if (strcmp(thresholdParams.method, 'svmGaussianRF')) || ...
        (strcmp(thresholdParams.method, 'mlptGaussianRF'))
    gaussianPoolingKernel = generateSpatialPoolingKernel(...
        rParams.spatialParams, rParams.mosaicParams, ...
        rParams.topLevelDirParams, p.Results.visualizeSpatialScheme, ...
        thresholdParams, constantParamsList);
    thresholdParams = modifyStructParams(thresholdParams, ...
...
if (p.Results.compute)
    ...
    if (strcmp(thresholdParams.method, 'svmV1FilterBank')) || ...
            (strcmp(thresholdParams.method, 'svmGaussianRF')) || ...
            (strcmp(thresholdParams.method, 'svmV1FilterEnsemble'))
        fprintf(['Computing performance for <strong>%s</strong> ', ...
            'emPaths using an <strong>%s</strong> classifier ', ...
            'operating on the raw <strong>%s</strong>.\n'], ...
            rParams.temporalParams.emPathType, thresholdParams.method, ...
            thresholdParams.signalSource);
    elseif (~strcmp(thresholdParams.method, 'mlpt'))
        fprintf(['Computing performance for <strong>%s</strong> ', ...
            'emPaths using an <strong>%s</strong> classifier ', ...
            'operating on the first %d PCA components of <strong>', ...
            '%s</strong>.\n'], rParams.temporalParams.emPathType, ...
            thresholdParams.method, thresholdParams.PCAComponents, ...
            thresholdParams.signalSource);
    else
        fprintf(['Computing performance for <strong>%s</strong> ', ...
            'emPaths using an <strong>%s</strong> classifier ', ...
            'operating on  <strong>%s</strong>.\n'], ...
            rParams.temporalParams.emPathType, thresholdParams.method, ...
            thresholdParams.signalSource);
    end
    ...
    ancillaryData = ...
        rwObject.read('ancillaryData', paramsList, readProgram);
    noStimData = ...
        rwObject.read('responseInstances', paramsList, readProgram);
    ...
    if isfield(thresholdParams, 'trialsUsed')
        noStimData.responseInstanceArray = keepTrialsUsed(...
            noStimData.responseInstanceArray, thresholdParams.trialsUsed);
    end

    % Only keep the time bins we will use
    if (~isempty(thresholdParams.evidenceIntegrationTime))
        [noStimData, thresholdParams.actualEvidenceIntegrationTime] = ...
            keepTimeBinsUsed(noStimData, ...
            thresholdParams.evidenceIntegrationTime);
    end
    ...
    % should be identical structs. Same for testDirectionParams and
    % ancillaryParams.instanceParams Check for that below.
    displayStructs = false;
    checkStructs('rParams', rParams, ...
        'ancillaryParams-rParams', ancillaryData.rParams, ...
        'ignoredFields', {'plotParams'}, 'displayStructs', displayStructs);
    checkStructs('testDirectionParams', testDirectionParams, ...
        'ancillaryParams.instanceParams', ancillaryData.instanceParams, ...
        'displayStructs', false);
    fprintf('done\n');
    ...
    %for kk = nParforConditions:-1:1
        ...
        fprintf('Classifying data for condition %d of %d... \n', kk, ...
            nParforConditions);
        stimData = rwObject.read('responseInstances', paramsList, ...
            readProgram);
        ...
        if isfield(thresholdParams, 'trialsUsed')
            stimData.responseInstanceArray = keepTrialsUsed(...
                stimData.responseInstanceArray, ...
                thresholdParams.trialsUsed);
        end

        % Only keep the time bins we will use
        if (~isempty(thresholdParams.evidenceIntegrationTime))
            [stimData, ~] = keepTimeBinsUsed(stimData, ...
                thresholdParams.evidenceIntegrationTime);
        end
        ...
        [usePercentCorrect(kk), useStdErr(kk), h, ...
            varianceExplained(kk, :)] = ...
            classifyForOneDirectionAndContrast(noStimData, stimData, ...
            thresholdParams, 'paramsList', paramsList, ...
            'parforWorkersNum', parforWorkersNum, ...
            'visualizeKernelTransformedSignals', ...
            p.Results.visualizeKernelTransformedSignals, ...
            'plotSvmBoundary', ...
            p.Results.generatePlots && p.Results.plotSvmBoundary, ...
        ...
        if (p.Results.generatePlots && p.Results.plotSvmBoundary)
            ...
            rwObject.write(sprintf('svmBoundary_PCA%d_PCA%d', ...
                plotSvmPCAAxis1, plotSvmPCAAxis2), h, paramsList, ...
                writeProgram, 'Type', 'figure');
            close(h);
        end
    end
    rng(rState);
    fprintf(['Classification for all %d conditions took %2.2f ', ...
        'minutes\n'], nParforConditions, toc / 60);
    clearvars('theData', 'useData', 'classificationData', 'classes');

    % Take the returned vector form of the performance data and put it back
    % into the matrix form we expect below and elsewhere.
    %
    % See function responseGenerationParforConditionStructsGenerate for how
    % we pack the conditions into the order that this unpacks.
    for kk = 1:nParforConditions
        thisConditionStruct = parforConditionStructs{kk};
        performanceData.percentCorrect(thisConditionStruct.ii, ...
            thisConditionStruct.jj) = usePercentCorrect(kk);
        performanceData.stdErr(thisConditionStruct.ii, ...
            thisConditionStruct.jj) = useStdErr(kk);
        performanceData.varianceExplained(thisConditionStruct.ii, ...
            thisConditionStruct.jj, :) = squeeze(varianceExplained(kk, :));
    end

    %% Tuck away other information that we want to store
    performanceData.IBIOColorDetectSnapshot = ...
    ...
    paramsList = constantParamsList;
    paramsList{numel(paramsList) + 1} = thresholdParams;
    rwObject.write('performanceData', performanceData, paramsList, ...
        writeProgram);
    fprintf('done\n');

    %% Validation data
    if (nargout > 0)
        validationData.testConeContrasts = ...
            performanceData.testConeContrasts;
...
%% Plot performances as raw psychometric functions in each color direction
if (p.Results.generatePlots && p.Results.plotPsychometric)
    fprintf('Reading performance data ... ');
    paramsList = constantParamsList;
    paramsList{numel(paramsList) + 1} = thresholdParams;
    performanceData = rwObject.read('performanceData', paramsList, ...
        writeProgram);
    fprintf('done\n');
    ...
    for ii = 1:size(testConeContrasts, 2)
        hFig = figure;
        clf;
        errorbar(testContrasts, squeeze(...
            performanceData.percentCorrect(ii, :)), ...
            squeeze(performanceData.stdErr(ii, :)), 'ro-', ...
            'LineWidth', rParams.plotParams.lineWidth, ...
            'MarkerSize', rParams.plotParams.markerSize, ...
            'MarkerFaceColor', [1.0 0.5 0.50]);
        axis 'square'
        set(gca, 'XScale', 'log', 'YLim', [0 1.0], ...
            'XLim', [testContrasts(1) testContrasts(end)], ...
            'FontSize', rParams.plotParams.axisFontSize);
        xlabel('contrast', ...
        ...
        if (isempty(thresholdParams.evidenceIntegrationTime))
             title(sprintf(['LMS = [%2.2f %2.2f %2.2f]\nsignal = ', ...
                 '''%s'', classifier = ''%s''\nemPath = ''%s'''], ...
                 testConeContrasts(1, ii), testConeContrasts(2, ii), ...
                 testConeContrasts(3, ii), ...
            ...
            title(sprintf(['LMS = [%2.2f %2.2f %2.2f]\nsignal = ', ...
                '''%s'', classifier = ''%s''\nemPath = ''%s'', ', ...
                'evidenceIntTime: %2.1f ms'], testConeContrasts(1, ii), ...
                testConeContrasts(2, ii), testConeContrasts(3, ii), ...
                 thresholdParams.signalSource, thresholdParams.method, ...
                 rParams.temporalParams.emPathType, ...
                 thresholdParams.actualEvidenceIntegrationTime), ...
                'FontSize', rParams.plotParams.titleFontSize);
        end
        rwObject.write(sprintf('performanceData_%d', ii), hFig, ...
...
if (p.Results.visualizeVarianceExplained) && (p.Results.compute)
    ...
    paramsList{numel(paramsList) + 1} = thresholdParams;
    performanceData = ...
        rwObject.read('performanceData', paramsList, writeProgram);
    fprintf('done\n');

    if (isfield(performanceData, 'varianceExplained')) && ...
            (~(all(isnan(performanceData.varianceExplained(:)))))
        testConeContrasts = performanceData.testConeContrasts;
        testContrasts = performanceData.testContrasts;
        % Plot variance explained figure
        for ii = 1:size(testConeContrasts, 2)
            hFig = figure(1200 + ii - 1);
            clf;
            set(hFig, 'Position', [10 10 960 1260], 'Color', [1 1 1]);
            hold on;
            pcaIndices = 1:size(performanceData.varianceExplained, 3);
            percentCorrect = ...
                squeeze(performanceData.percentCorrect(ii, :));
            legends = {};
            colors = brewermap(numel(testContrasts), '*Spectral');
            for contrastIndex = numel(testContrasts):-1:1
                variances = squeeze(performanceData.varianceExplained(...
                    ii, contrastIndex, :));
                plot(pcaIndices, cumsum(variances), 'ko-', ...
                    'MarkerSize', 6, 'MarkerFaceColor', ...
                    squeeze(colors(contrastIndex, :)), ...
                    'MarkerFaceColor', ...
                    squeeze(colors(contrastIndex, :)), ...
                    'Color', squeeze(colors(contrastIndex, :)), ...
                    'LineWidth', 1.5);
                legends{numel(legends) + 1} = sprintf(...
                    'contrast:%2.1e, %%correct:%2.0f)', ...
                    testContrasts(contrastIndex), ...
                    100.0 * percentCorrect(contrastIndex));
            end
            hL = legend(legends, 'Location', 'SouthEast');
        end
        set(gca, 'XScale', 'log', 'YScale', 'log', ...
            'XLim', [1 numel(pcaIndices)], 'YLim', [0.1 100], ...
            'XTick', [1 10 60 100 600 1000], 'YTick', [0.1 1 10 100], ...
            'FontSize', 18);
        xlabel('# of PCA components', 'FontWeight', 'bold');
        ylabel('cumulative response variance explained (%)', ...
            'FontWeight', 'bold');
        title(sprintf('%d instances, %2.0f c/deg', ...
            thresholdParams.trialsUsed, ...
            rParams.spatialParams.cyclesPerDegree));
        box('on');
        grid('on');
    else
       fprintf(['Did not find varianceExplained field in ', ...
           'performanceData. No plotting.\n']);
    end
end
...
% Check the equality of the structures
%
% Syntax:
%   checkStructs(struct1Name, struct1, struct2Name, struct2, [varargin])
%
% Description:
%    Check the provided structures for equality
%
% Inputs:
%    struct1Name    - String. The name of the first structure.
%    struct1        - Struct. The first structure to compare.
%    struct2Name    - String. The name of the second structure.
%    struct2        - Struct. The second structure to compare.
%
% Outputs:
%    None.
%
% Optional key/value pairs:
%    ignoredFields  - Cell. A cell array of fields to ignore. Default {}.
%    displayStructs - Boolean. Whether or not to display the structures.
%                     Default false.
    ...
    if (p.Results.displayStructs)
        disp(UnitTest.displayNicelyFormattedStruct(...
            struct1, struct1Name, '', 60));
        disp(UnitTest.displayNicelyFormattedStruct(...
            struct2, struct2Name, '', 60));
    end
...
%             fprintf(2, '\t[%d]. %s\n', k, structCheck{k});
%         end
%         error(['\n<strong>%s and %s are NOT identical structs. ', ...
...
function responseInstanceArray = ...
    keepTrialsUsed(responseInstanceArray, trialsUsed)
% Keep the used trials
%
% Syntax:
...
%
% Description:
%    Keep the used trials, and remove the unused trials from the array.
%
% Inputs:
%    responseInstanceArray - Struct. A response instance structure.
%    trialsUsed            - Vector. A numeric vector of the trials used.
%
% Outputs:
%    responseInstanceArray - Struct. The modified input structure.
%
% Optional key/value pairs:
%    None.
%
    ...
    if (~isempty(responseInstanceArray.theMosaicIsomerizations))
        fprintf('Classifying based on %d of the computed %d trials\n', ...
            trialsUsed, size(...
            responseInstanceArray.theMosaicIsomerizations, 1));
    ...
    if ~isempty(responseInstanceArray.theMosaicPhotocurrents)
        fprintf('Classifying based on %d of the computed %d trials\n', ...
            trialsUsed, size(...
            responseInstanceArray.theMosaicPhotocurrents, 1));
...
function [stimData, actualEvidenceIntegrationTime] = ...
    keepTimeBinsUsed(stimData, evidenceIntegrationTime)
% Keep the used time bins.
%
% Syntax:
%   [stimData, actualEvidenceIntegrationTime] = ...
%       keepTimeBinsUsed(stimData, evidenceIntegrationTime)
%
% Description:
%    Keep the used time bins.
%
% Inputs:
%    stimData                      - Struct. A stimulus data structure.
%    evidenceIntegrationTime       - Numeric. The integration time for the
%                                    evidence, in milliseconds.
%
% Outputs:
%    stimData                      - Struct. The modified stimulus data.
%    actualEvidenceIntegrationTime - Numeric. The actual evidence
...
%
    ...
    dt = stimData.responseInstanceArray.timeAxis(2) - ...
        stimData.responseInstanceArray.timeAxis(1);
    timeBinsToKeep = find(stimData.responseInstanceArray.timeAxis <= ...
        stimData.responseInstanceArray.timeAxis(1) + ...
        abs(evidenceIntegrationTime) / 1000);
    if (evidenceIntegrationTime >= 0)
        % em1 -> emN
        actualEvidenceIntegrationTime = numel(timeBinsToKeep) * dt * 1000;
    else
        % emN -> em1
        timeBinsToKeep = numel(...
            stimData.responseInstanceArray.timeAxis) - timeBinsToKeep + 1;
        actualEvidenceIntegrationTime = -numel(timeBinsToKeep) * dt * 1000;
    end

    if (~isempty(responseInstanceArray.theMosaicIsomerizations))
        responseDimensionality = ndims(...
            stimData.responseInstanceArray.theMosaicIsomerizations);
        ...
        elseif (responseDimensionality == 3)
            ...
            stimData.noiseFreeIsomerizations = ...
                stimData.noiseFreeIsomerizations(:, timeBinsToKeep);
    ...
    if (~isempty(responseInstanceArray.theMosaicPhotocurrents))
        ...
        elseif (responseDimensionality == 3)
            ...
            stimData.noiseFreePhotocurrents = ...
                stimData.noiseFreePhotocurrents(:, timeBinsToKeep);
        elseif (responseDimensionality == 4)
            % nTrials X cone rows x cone cols x time bins
            stimData.responseInstanceArray.theMosaicPhotocurrents = ...
                stimData.responseInstanceArray.theMosaicPhotocurrents(...
